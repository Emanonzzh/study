...
- `项目口述稿_遥感监测智能助手.md` —— 面试口述稿（1 分钟版/3 分钟版/追问 Q&A/踩坑故事/证据索引）；**仓库内同步版：`docs/项目口述稿.md`**
...
### 求职结论（不变）
- 26 届已毕业 → 走**社招/往届**通道（2026.9 秋招面向 27 届，不适用）；深圳、行业不限
- 双轨：**AI 应用开发（遥感+AI 差异化）主攻**；GIS 内业 / 技术支持 / 测试作保底上车
- 卡点 = 无工作经验 + 非科班 + 手写代码弱 → 靠"公网 Demo + 投递量 + 手写基础 + 面试准备"补

### ⏳ 待办（白天，按序）
1. **服务器部署新版**（线上仍是旧版，面试官会点简历里的链接）：ECS `root@47.76.101.97` → 项目目录 → `git pull` → `bash deploy.sh`
