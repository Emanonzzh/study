...
### 求职材料
- **已在仓库内（两台电脑共用）**：
  - `docs/学习路径_校准版.md` —— ⭐ **唯一权威学习路径**（9 份真实 JD 词频校准 + 7 周路径 + 不做清单 + 验收标准）；周表以它为准
  ...
  - `docs/开发转型计划_Agent开发与AI运维.md`、`docs/Agent项目改进路线.md` —— 早期计划与项目改进优先级（已被《学习路径_校准版》覆盖）
  - **`docs/求职/`** —— 岗位与投递运营：`A层岗位清单.md`、`社招岗位巡检清单.md`（含 3 次巡检记录）、`校招窗口巡检清单.md`、`投递记录表.csv`、`求职打招呼话术.md`、`深圳投递公司清单.md`
