...
### 求职材料
- **已在仓库内（两台电脑共用，`docs/` 目录）**：
  - `docs/项目口述稿.md` —— 面试口述稿（1 分钟版/3 分钟版/追问 Q&A/踩坑故事/证据索引）
  - `docs/手写练习_题单与答案.md` —— **手写题单（3 组题）+ 参考实现**（先写再看答案；每天 3 题）
  - `docs/开发转型计划_Agent开发与AI运维.md` —— 6 周深度计划 + **2 周冲刺版（能面能投）** + Python 速成清单 + CRUD + code review 训练法
  - `docs/Agent项目改进路线.md` —— **项目改进优先级 P0-P3**（P0=可讲性+工程基本盘，P1=Agent 深度，P2=运维向，P3=差异化；含"别做"清单与明天开工顺序）
  - `docs/SQL增删改查入门.md` —— SQL 入门（用 monitoring.db 真实表结构，含完整增改删示例与常见坑）
- **仅在本机 `C:\Users\123\Desktop\WORK\`（含个人信息，不上仓库）**：
  - `钟英浩_AI应用开发_简历_深圳版.md`（已含 V2-C 成果：检索命中率 83%→100%）
  - `钟英浩_GIS内业_简历_深圳版.md`、`求职打招呼话术.md`、`深圳投递公司清单.md`
- **上传规则**：只上传"题单/计划/口述稿"这类无个人信息的学习材料；**简历、话术、投递清单不上仓库**
